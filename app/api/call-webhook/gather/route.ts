...
function buildGather(
  ...
  return `<Gather input="speech" language="${sttLang}" timeout="8" speechTimeout="3" enhanced="true"${hintAttr} action="${escapeXml(gatherUrl)}" method="POST">${say(questionText, lang, voice)}</Gather><Redirect method="POST">${escapeXml(fallbackUrl)}</Redirect>`;
}
